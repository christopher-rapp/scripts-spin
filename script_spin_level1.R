...

{
...

      {
...

        {
          ...
          # Bin formatting
          {
            ...
            {
              #Y Unit conversion from counts to concentration
  ...
  if (PLOT.ON == T){
    ...
    # Flatten lists and remove failed model runs
    {
      ...
      remove.c <- which(map(model.ls, 6) == "FAIL")

      if (length(remove.c) != 0){

        # Filter failed models
        model.ls <- model.ls[-remove.c]
      }
    }
    ...
    for (n in 1:length(data.ls)){
...

      {
...

        {
          ...
          plot.gg <- ggplot(tmp.long, aes(fill = `ML Class`, y = `Value`, x = `Variable`)) +
            ...
            ylab(label = "Count") +
            xlab(label = latex2exp::TeX(r'(Diameter ($\mu m))')) +
            theme_minimal() +
            theme(
...

        {
          ...
          plot.filename <- paste0(export.plot, date.c, " Classification Performance ", "Experiment ", ID, ".png")
          ...
          # Top row
          {
            gg1 <- ggplot(dataSPIN.df, aes(x = `OPC Size`, y = `Depolarization`, col = `Class`)) +
              ...
              theme(
                ...
                plot.margin = unit(c(0.2, -1, 0.25, 0.2), "cm"),
                aspect.ratio = 1
          ...
          total.gg <- top.gg / bot.gg + plot_layout(guides = "collect") +
            plot_annotation(title = title.main, subtitle = title.sub)
...

        {
          # Model Performance
          {
            ...
            plot.filename <- paste0(export.plot.path, date.c, " ML Model Performance", " Experiment ", ID, ".png")
          ...
          # Model Performance
          {
            ...
            plot.filename <- paste0(export.plot.path, date.c, " PCA Model Performance", " Experiment ", ID, ".png")
        ...
        {
          print(paste0(date.c, ": Light Scattering"))

          tmp.df <- dataSPIN.df %>%
            filter(`Inlet Filter ON` == 0)

          title.main <- paste0("Spectrometer for Ice Nucleation (SPIN)")
          title.sub <- paste0("Light Scattering: ", date.c, ", Experiment ", ID)

          plot.filename <- paste0(export.plot.path, date.c, " Light Scattering", " Experiment ", ID, ".png")
          ...
          gg1 <- ggplot(tmp.df, aes(y = `Depolarization`, x = after_stat(scaled), fill = as.factor(`Lamina Breaks`), color = as.factor(`Lamina Breaks`), group = `Lamina Breaks`)) +
            ...
            annotate("text", y = 0.9, x = 0.9, label = "(a)", fontface = "bold", size = 12/.pt) +
          ...
          gg2 <- ggplot(tmp.df, aes(x = `Lamina S Ice`, y = `Depolarization`, color = as.factor(`Lamina Breaks`), group = `Lamina Breaks`)) +
            annotate("rect", ymin = 0.4, ymax = 1, xmin = 1, xmax = 1.6, fill = "gray60", alpha = 0.2) +
            annotate("rect", ymin = 0.16, ymax = 0.4, xmin = annotation.df[3, 2], xmax = 1.6, fill = "darkseagreen2", alpha = 0.2) +
            annotate("rect", ymin = 0, ymax = 0.16, xmin = 1, xmax = 1.6, fill = "gray60", alpha = 0.2) +
            geom_point(size = 0.1, alpha = 0.1, show.legend = F) +
            ...
            scale_color_manual(values = c("#E69F00", "#999999", "#56B4E9", "#009E73")) +
            annotate("text", y = 0.9, x = 1.55, label = "(b)", fontface = "bold", size = 12/.pt) +
            annotate("text", y = 0.55, x = 1.5, label = "Ice", hjust = 0, size = 8/.pt) +
            annotate("text", y = 0.28, x = 1.5, label = "Droplet", hjust = 0, size = 8/.pt) +
          ...
          plot.gg <- gg1 + gg2 + plot_layout(guides = "collect") +
            plot_annotation(title = title.main, subtitle = title.sub) &
      ...
      # ---------------------------------------------------------------------- #
      ##### SUBSECTION: Activation Quicklook #####
      #'

      {
        print(paste0(date.c, ": Activation Quicklook"))

        title.main <- paste0("Spectrometer for Ice Nucleation (SPIN)")
        title.sub <- paste0("Activation Quicklook: ", date.c, ", Experiment ", ID)

        # Subset
        tmp.df <- dataSPIN.df

        if (sum(tmp.df$`Total Background`, na.rm = T) == 0){

          # If there is very limited data for backgrounds apply correction factor as the total concentration
          tmp.df$`Total Background` <- 1.4
        }

        tmp.df <- tmp.df %>%
          mutate(`Total Size Ice` = `Total Size Ice` - `Total Background`, .after = `Total Background`) %>%
        ...
        # Smooth the activation curves grouped by temperature and ordered by Sice so both ramps are considerede back to back
        tmp.df <- tmp.df %>%
          group_by(`Lamina Breaks`) %>%
          arrange(`Lamina S Ice`) %>%
          mutate(`Total Size Ice` = zoo::rollmean(`Total Size Ice`, k = 10, fill = NA)) %>%
          arrange(`UTC Time`) %>%
          ungroup()

        # Filter empty ramp data typically generated if the instrument is restarted during an experiment
        {
          tmp.df <- tmp.df %>%
            group_by(`Lamina Breaks`) %>%
            mutate(`Total Size Ice` = case_when(all(is.na(`Total Size Ice`)) ~ 9999, !all(is.na(`Total Size Ice`)) ~ `Total Size Ice`)) %>%
            ungroup()
        ...
        # Reorder classes so each plot is the same format
        tmp.df$Class <- factor(tmp.df$Class, ordered = T, levels = c("Aerosol", "Water Uptake", "Ice", "Droplet"))
        ...
        # Sigmoid Curve Fitting
        {
          ...
          tryCatch(expr = {
              ...
              sigmoid.ls <- lapply(sigmoid.ls, function(x){
                ...
                mid.nm <- data.frame(`Lamina S Ice` = model$m$getAllPars()[[2]], `Total Size Ice` = model$m$getAllPars()[[1]]/2, check.names = F)
              })
        ...
        plot.filename <- paste0(export.plot, date.c, " Activation Quicklook", " Experiment ", ID, ".png")

        if (FIT.break == F){

          plot.gg <- ggplot(tmp.df, aes(x = `Lamina S Ice`, y = `Total Size Ice`, col = `Class`)) +
            labs(title = title.main, subtitle = title.sub) +
            geom_point(alpha = 0.5) +
            ylab(label = latex2exp::TeX(r'(OPC Concentration ($D_p$ \geq 2.5 \mu{m}))')) +
            scale_x_continuous(breaks = seq(1, 1.7, 0.1), limits = c(1, 1.7), minor_breaks = seq(1, 1.7, 0.025)) +
            geom_smooth(
              method = "nls",
