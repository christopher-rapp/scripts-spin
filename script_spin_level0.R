  ...
  for (n in 1:length(spin.dates)){

    {
      ...
      # Overview plot
      {
        ...
        rect.df <- dataSPIN.df %>%
          select(`Local Time`, `Inlet Filter ON`) %>%
          mutate(`Filter ID` = rleid(`Inlet Filter ON`),
                 .before = everything()) %>%
          group_by(`Filter ID`) %>%
          mutate(first = first(`Local Time`),
                 last = last(`Local Time`)) %>%
          distinct(`Filter ID`, .keep_all = T) %>%
          mutate(`Inlet Filter ON` = as.factor(`Inlet Filter ON`)) %>%
          filter(`Inlet Filter ON` == 0) %>%
          ungroup() %>%
          select(-c(`Filter ID`, `Local Time`))

        vline.df <- data.frame(x = c(sequence.evrmp.tm, sequence.icing.tm, sequence.ramps.tm),
                               name = c(rep("Evaporation Sequence", length(sequence.evrmp.tm)),
                                        rep("Icing Sequence", length(sequence.icing.tm)),
                                        rep("Ramp Sequence", length(sequence.ramps.tm))))

        overview.gg <- ggplot(dataSPIN.df, aes(x = `Local Time`)) +
          geom_rect(
            data = rect.df,
            aes(
              xmin = `first`,
              ...
              ymin = -Inf,
              ymax = +Inf,
              fill = `Inlet Filter ON`
            ),
            inherit.aes = F,
            alpha = 0.5
          ) +
          geom_line(
            aes(y = `Evaporation SP`, color = "Evaporation Set Point"),
            linetype = 1,
            linewidth = 1.1
          ) +
          geom_line(
            aes(y = `Warm Wall SP`, color = "Warm Wall Set Point"),
            linetype = 1,
            linewidth = 1.1
          ) +
          geom_line(
            aes(y = `Cold Wall SP`, color = "Cold Wall Set Point"),
            linetype = 1,
            linewidth = 1.1
          ) +

          {
            if (nrow(dataSPIN.df) != length(which(is.na(dataSPIN.df$`Lamina Temperature`)))) {
              # Only plot this data if it is non-empty
              geom_line(
                aes(y = `Lamina Temp (C)`, color = "Lamina Temperature"),
                linetype = 1,
                linewidth = 1.1
              )
            }
          ...
          scale_y_continuous(n.breaks = 10, expand = c(0.1, 0.2)) +
          scale_fill_manual(values = c("Inlet Filter ON" = "gray48")) +
          scale_color_manual(
            values = c(
              "Warm Wall Set Point" = "firebrick1",
              "Cold Wall Set Point" = "dodgerblue1",
              "Lamina Temperature" = "mediumorchid3",
              "Evaporation Set Point" = "green",
              "Ramp Sequence" = "black",
              "Evaporation Sequence" = "goldenrod1",
              "Icing Sequence" = "lightseagreen"
            )
          ) +
        ...
        rm(vline.df, rect.df, overview.gg)
      }
...

      {
        ...
        if (nrow(dataPBP.df) != 0){
          ...
          if (length(sequence.ramps.tm) == 0){
            ...
            print(paste0("Plotting: Particle Time Series"))
